...
max_freq=$(cat /sys/devices/system/cpu/cpufreq/policy4/cpuinfo_max_freq)
echo $max_freq > /sys/devices/system/cpu/cpufreq/policy4/scaling_max_freq
max_freq=$(cat /sys/devices/system/cpu/cpufreq/policy7/cpuinfo_max_freq)
echo $max_freq > /sys/devices/system/cpu/cpufreq/policy7/scaling_max_freq
echo "cpufreq set to maximum value: $max_freq"

# Set cpufreq to minimum value
min_freq=$(cat /sys/devices/system/cpu/cpufreq/policy0/cpuinfo_min_freq)
echo $min_freq > /sys/devices/system/cpu/cpufreq/policy0/scaling_min_freq
min_freq=$(cat /sys/devices/system/cpu/cpufreq/policy4/cpuinfo_min_freq)
echo $min_freq > /sys/devices/system/cpu/cpufreq/policy4/scaling_min_freq
min_freq=$(cat /sys/devices/system/cpu/cpufreq/policy7/cpuinfo_min_freq)
echo $min_freq > /sys/devices/system/cpu/cpufreq/policy7/scaling_min_freq
echo "cpufreq set to minimum value: $min_freq"
...
# Disable Hotplug
for i in /sys/devices/system/cpu/cpu*/core_ctl; do
   echo 0 > $i/enable
done
...
# ข้อความ
su -lp 2000 -c "cmd notification post -S bigtext -t '🔥TWEAK🔥' 'Tag' 'Performance ⚡ปรับแต่ง⚡ CPU GPU FPS Stability Successfull FixBy@RealHard️'"
