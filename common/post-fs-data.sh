...
if [ "$TOGGLE_3D" -eq 1 ]; then
	...
	#setprop debug.force.hardware 1
	setprop debug.sf.frame_buffer_acquired_buffers 4
	setprop debug.hwui.renderer none
	setprop ro.HOME_APP_ADJ 1
	setprop debug.hwui.profile.maxframes -1
	setprop SurfaceFlinger.max_frame_rate -1
	setprop ro.hwui.render_ahead 4
    ...
    # Set 3D Optimization
    setprop log.tag.TackManagerService ERROR
    #setprop log.tag.MagiskManagerService ERROR
