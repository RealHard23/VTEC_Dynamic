...
su -c settings put global config_per_process_gpu_memory_fraction 0.8
...
# Late props which must be set after boot_completed
{
    ...
    # SafetyNet/Play Integrity | Avoid breaking fingerprint scanners
    ...
    # SafetyNet/Play Integrity | STRONG | Hide *first_api_level* in system to open access for *DEVICE_INITIAL_SDK_INT*
    if [[ "$(getprop ro.product.first_api_level)" -le 33 ]]; then
        resetprop ro.product.first_api_level 33
    else
        sleep 10
        resetprop --delete ro.product.first_api_level
    fi
}&
